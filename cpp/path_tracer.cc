...
PathTracer::Path PathTracer::operator()(const Ray& r, float Ni) {
  ...
  /* No intersection, returns an invalid path with 0 probability. */
  if (isnan(normal.x()) || isnan(normal.y()) || isnan(normal.z()))
    return PathTracer::Path(r, normal, 0, mtl);
  ...
  if (mtl.Tr == 0) {
    if (cos_omega > 0) {
      ...
      /* Intersects at a back surface, returns an invalid path with 0 probability. */
      return PathTracer::Path(r, normal, 0, mtl);
    }
  }
